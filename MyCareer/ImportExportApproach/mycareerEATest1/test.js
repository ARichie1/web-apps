import{toggleStyle, toggleContent, uniqueRangeGenerator,
        arrayRemove, camalize} from "./functions.js"
import{traits, careers} from "./traits.js"
...
const genTraitIndex = uniqueRangeGenerator(30,0,29);
const genPersonalityIndex = uniqueRangeGenerator(4,0,3);
...
traitBox1.innerHTML = trait[genPersonalityIndex[1]][genTraitIndex[personaityInc]];
traitBox2.innerHTML = trait[genPersonalityIndex[3]][genTraitIndex[personaityInc]];
traitBox3.innerHTML = trait[genPersonalityIndex[0]][genTraitIndex[personaityInc]];
traitBox4.innerHTML = trait[genPersonalityIndex[2]][genTraitIndex[personaityInc]];
...
// PERSONALITY THE CHOSEN TRAIT REPRESENTS OR FALL UNDER. 
    for (let i = 0; i < chosenWords.length; i++) {
        if (trait[0].includes(chosenWords[i])) {
            personalities[0].push(chosenWords[i]);
        }
        if (trait[1].includes(chosenWords[i])) {
            personalities[1].push(chosenWords[i]);
        }
        if (trait[2].includes(chosenWords[i])) {
            personalities[2].push(chosenWords[i]);
        }
        if (trait[3].includes(chosenWords[i])) {
            personalities[3].push(chosenWords[i]);
        }
    }
...
// RE-ARRANGE THE (pLengthArray) FROM HIGHEST TO LOWEST NUMBER
// THIS WILL HELP TO IDENTIFY DOMINANCE IN EACH PERSONALITY.
    personalitiesList = pLengthArray.sort((a, b) => b - a);
