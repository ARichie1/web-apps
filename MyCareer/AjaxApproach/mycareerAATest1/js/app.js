import{toggleStyle, toggleContent, uniqueRangeGenerator,
    arrayRemove, camalize, removeDuplicates} from "./functions.js"
...
// let genPersonalityIndex = uniqueRangeGenerator(4,0,3);
...
for (let i = 0; i < chosenWords.length; i++) {
    for (let j = 0; j < trait.length; j++) {
        if (trait[j].includes(chosenWords[i])) {
